...
# Clean data: Remove rows with Region_ID not in range [1, 15] or angle > 360
...
# Custom Dataset class with better feature engineering
class RegionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            # Create a blank image if the file can't be loaded
            image = Image.new('RGB', (224, 224))
        
        if self.transform:
            image = self.transform(image)
        
        # Get label (Region_ID - 1 to make it 0-indexed for CrossEntropyLoss)
        label = self.df.iloc[idx]['Region_ID'] - 1
        
        # Better feature engineering for time
        ...
        # Encode time with cyclical features to respect its circular nature
        ...
        # Normalize angle and also apply cyclical encoding
        angle = self.df.iloc[idx]['angle']
        angle_sin = np.sin(2 * np.pi * angle / 360)
        angle_cos = np.cos(2 * np.pi * angle / 360)
        
        # Package features with better encoding
        metadata = torch.tensor([
            time_sin, time_cos,      # Cyclical time encoding
            latitude, longitude,     # Normalized coordinates
            angle_sin, angle_cos     # Cyclical angle encoding
        ], dtype=torch.float32)
...
class RegionClassifier(nn.Module):
    def __init__(self, num_classes=15):
        ...
        # Load pre-trained EfficientNet B3 (higher capacity than B0)
        ...
        # Process metadata with more capacity - updated for 6 features
        self.metadata_encoder = nn.Sequential(
            nn.Linear(6, 64),  # Now 6 features: sin/cos time, lat, lon, sin/cos angle
            nn.BatchNorm1d(64),
            nn.ReLU(),
            nn.Dropout(0.3),
            nn.Linear(64, 128),
        ...
        # Combined classifier with batch normalization
    ...
    def forward(self, x, metadata):
        ...
        # Process metadata with enhanced architecture
...
# Smooth the weights to prevent over-emphasizing rare classes
class_weights = np.sqrt(class_weights)  # Square root smoothing
class_weights = class_weights / class_weights.sum() * len(class_weights)  # Normalize
class_weights = torch.FloatTensor(class_weights).to(device)

# Weighted loss with label smoothing for better generalization
criterion = nn.CrossEntropyLoss(weight=class_weights, label_smoothing=0.1)

# Use a more advanced optimizer with weight decay for regularization
optimizer = optim.AdamW([
    {'params': model.efficientnet.parameters(), 'lr': 0.0001},  # Lower learning rate for pre-trained parts
...
def train_model(model, train_loader, val_loader, criterion, optimizer, num_epochs=20):
    ...
    # Add a buffer to ensure we don't run out of steps (multiple by 1.1 for safety)
    total_steps = int(len(train_loader) * num_epochs * 1.1)
    
    # Use One Cycle learning rate scheduler with the updated total_steps
    ...
    for epoch in range(num_epochs):
        ...
        for images, metadata, labels in train_bar:
            ...
            # Automatic mixed precision (faster training with less memory)
            ...
            # Update scheduler every batch (for OneCycleLR)
            scheduler.step()
        ...
        # Early stopping check - but ensure we've done at least 10 epochs
...
# Function to predict Region_ID for validation and test sets
def predict_region_ids():
    ...
    with torch.no_grad():
        ...
        for images, metadata, _ in val_bar:
            ...
            # Accumulate predictions with multiple augmentations (Test Time Augmentation)
            # Original image
            outputs = model(images, metadata)
